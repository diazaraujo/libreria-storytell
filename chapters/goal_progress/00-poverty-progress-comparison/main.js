/**
 * poverty_progress_comparison — ranked bars (single-year series auto-fix)
 */
window.AtlasReplica = {
  ...
  async render(scene, ctx) {
    ...
    hidePlaceholder();
    const rows = await AtlasLoad.csv("./data/progress_data_poverty_slopecharts.csv");
    const NAMES = window.ATLAS_COUNTRY_NAMES || {};
    const headers = rows[0] ? Object.keys(rows[0]) : [];
    const catKey = headers.find(h => /iso3c|country|name|region|code|group/i.test(h)) || headers[0];
    const valKey = headers.find(h => /value|rate|score/i.test(h)) || headers.find(h => h !== catKey && rows.some(r => Number.isFinite(+r[h])));
    let data = rows.map(r => ({
    ...
    if (sceneIndex === 0 && data.length > 50) data = data.slice(0, 40);
    else if (sceneIndex >= 1 && data.length > 50) data = data.slice(-40);
    ...
    const h = Math.max(root.clientHeight || 440, 40 + data.length * 12);
    const margin = { top: 20, right: 56, bottom: 20, left: 130 };
